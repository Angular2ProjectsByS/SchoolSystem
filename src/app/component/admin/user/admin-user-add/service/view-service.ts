...
export class ViewService {
    ...
    checkFormCorrection(): boolean {
        let isFormOk = true;
        console.log(this.isFormInputOk);
        for (const isInputOk of this.isFormInputOk) {
            if (!isInputOk) {
                isFormOk = false;
            }
        }
    ...
    validateStreetName(streetName: string) {
        const isValidOk = this.validService.validNameStartGreat(streetName);
    ...
    validateHouseNumber(houseNumber: string) {
        const isValidOk = this.validService.validNumber(houseNumber);
    ...
    validateApartmentNumber(apNumber: string) {
        const isValidOk = this.validService.validNumber(apNumber);
    ...
    validateVoivodeshipName(name: string) {
        console.log(name);
        const isValidOk = this.validService.validName(name);
    ...
    validatePhoneNumber(number: string) {
        const isValidOk = this.validService.validPhoneNumber(number);
        this.isFormInputOk[9] = isValidOk;
        this.changeValidErrorDivDisplay(isValidOk, 'phone-number-invalid');
    }

    validateEmail(email: string) {
        const isValidOk = this.validService.validEmail(email);
